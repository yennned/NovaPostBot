...
(У каждого разработчика — свой клон; путь выше — канонический для этого репозитория,
не подменять его путём чужого клона.)
...
- **Фазы 0–7 закрыты все.** Дальше работа идёт не фазами, а отдельными задачами
  (багфиксы, хардening, эксплуатация) — правило «одна ветка на задачу, PR в
  защищённый `main`» не меняется.
...
PR-гигиена при этом: крупную задачу довести **несколькими мелкими PR одного
владельца** (а не одним big-bang).
