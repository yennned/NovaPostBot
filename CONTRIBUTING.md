...
- [ ] `ruff check .` **и** `ruff format --check .` (CI гоняет именно `--check`).
- [ ] `pytest` зелёный (реальный Postgres, **не** дефолтная dev-БД — см. «Линт и тесты»).
- [ ] Коммиты точечные, conventional; секретов в диффе нет.
- [ ] `PROGRESS.md` дополнен.
- [ ] **Docs синхронизированы с правкой логики.** Если изменилось поведение,
      описанное в [`docs/`](docs/), — правим документ в том же PR. Если функция
      сознательно не сделана, она идёт в раздел «Что осознанно не реализовано»
      [`docs/ROADMAP.md`](docs/ROADMAP.md), а не описывается как работающая.
      Расхождение «документ описывает намерение, код делает другое» — это баг
      документа, и стоит он дороже опечатки: по нему принимают решения.
...
Рантайм — **3.14** (`Dockerfile`, оба джоба CI, `.python-version` для pyenv/uv).
`requires-python = ">=3.12"` и ruff `target-version = "py312"` в `pyproject.toml`
занижены **намеренно**: иначе ruff-format эмитит PEP 758-синтаксис, который 3.12
и 3.13 не парсят. Не поднимать их «для порядка».

Практическое следствие: до `.python-version` контрибьютор на 3.12 получал
полностью зелёный локальный прогон при проде на 3.14. Файл это закрывает для
pyenv и uv — но он лишь подсказка, а не гарантия: сверяйте `python -V`.
...
**Compose едет вместе с кодом.** Источник правды — `docker-compose.yml` **в
репозитории**; правки на сервере вручную не делаем. Деплой копирует файл в
`$DEPLOY_PATH/.deploy-incoming/`, затем SSH-шаг:

1. сохраняет текущий как `docker-compose.yml.bak`;
2. подменяет и проверяет `docker compose config -q` **до** `pull`/`up`;
3. при любом сбое (невалидный конфиг, упавший `up`, не поднявшийся `bot`/`worker`)
...
> задан `APP_IMAGE`, и в серверном compose нет прод-специфики, которой нет в
> репозиторном (доп. порты, volume, отдельный сервис). Если специфика есть — сначала
> переносим её в репозиторный файл, иначе первый деплой применит конфиг без неё.
> Сервис `postgres` в репозиторном compose под профилем `dev` и в проде не
> поднимается — прод-БД внешняя (Neon).
