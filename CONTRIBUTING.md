...
## Модель работы — последовательно по фазам (актуально с 2026-06-18)
...
- Закреплённая очередь сейчас: **Phase 2 → alex**, **Phase 3 → step**,
  **Phase 4 → alex**. Дальше по умолчанию чередуемся:
  **Phase 5 → step**, **Phase 6 → alex**, **Phase 7 → step**,
  если не появится отдельное решение перед стартом следующей фазы.
