...
def description_cleaned(cleaned: str) -> str:
    """Часть символов Нова Пошта в описании не принимает — показать, что уйдёт.

    Молча подменять введённое нельзя: описание печатается на ярлыке, и клиент
    должен увидеть итог до отправки, а не после.
    """
    return (
        "ℹ️ Нова Пошта не приймає деякі символи в описі. Буде надіслано:\n"
        f"<b>{html.escape(cleaned)}</b>"
    )
...
def card_text(data: dict, price: dict) -> str:
    ...
    if price.get("unavailable"):
        ...
        if price.get("billable_weight"):
            # Объёмный вес перебил фактический — иначе клиент не поймёт, почему
            # цена выше ожидаемой по весу (НП тарифицирует по максимуму из двух).
            lines.append(
                f"   Тарифна вага: {price['billable_weight']} кг (об'ємна, за габаритами коробки)"
            )
