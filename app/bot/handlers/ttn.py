...
async def cb_cart_edit(
    ...
    if match is not None and match.available <= 0:
        # Товар кончился, пока позиция лежала в кошику: степпер с максимумом 0
        # нерабочий (шаг «−1» не опускается ниже 1). Как и `cb_pick`, не молчим —
        # перерисовываем кошик и говорим, что делать.
        await _show_cart(callback.message, state)
        await callback.answer(
    ...
    # реального наличия. Если позиции в остатках нет вовсе (`match is None`) —
    # держимся за количество из кошика, тогда её можно только уменьшить.
