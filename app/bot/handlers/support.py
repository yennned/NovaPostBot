"""Хендлеры поддержки (Фаза 6): релей-чат клиент↔дежурный + лог для owner/dev.

Клиент пишет через «💬 Звернення до менеджера» — сообщения релеятся дежурному.
Дежурный/владелец видят «💬 Підтримка»: менеджер — свой инбокс + очередь без
дежурного, владелец/dev — полный лог с поиском. Релей идёт только через бота,
прямых Telegram-контактов нет ([docs/10-support-duty.md]).
...
def _is_staff(ctx: EffectiveContext) -> bool:
    return ctx.effective_role in {UserRole.manager, UserRole.owner} or ctx.is_dev
...
async def client_start(
    ...
    if result.notify_owner:
        await notifications.notify_support_queued_to_owner(
...
async def cb_search(
    ...
    if callback.message is None or not _is_owner_or_dev(effective_context):
        await callback.answer(_STALE, show_alert=True)
        return
...
async def staff_search_input(
    ...
    await state.clear()
    if not _is_owner_or_dev(effective_context):
        return
    await _show_inbox(
