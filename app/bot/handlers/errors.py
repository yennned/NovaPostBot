...
async def on_unhandled_error(event: ErrorEvent) -> None:
    """Последний backstop: любое НЕОЖИДАННОЕ исключение.
    ...
    последний в `build_dispatcher`, так что этот backstop — действительно край.
    """
    callback = event.update.callback_query
    # Только `getattr` с дефолтом: у `Update` нет `event_from_user` (это ключ в
    # `data`, а не поле модели), и прямое обращение роняло сам backstop — то есть
