...
async def check_server_version(session: AsyncSession) -> str:
    ...
    Зовётся только из `app/main.py`: воркеру своя проверка не нужна — он ходит в
    ту же базу, и второй `SHOW server_version` на старте ничего не добавил бы.
    (Прежнее объяснение — «чтобы ночью Neon засыпал» — не выдержало проверки:
    дворник броней ходит в БД каждые 120 с круглосуточно, см. `app/worker.py`.)
    """
