...
/**
 * Предупреждение при правке журнала. Именно warningOnly, а не запрет.
 *
 * Строки сюда пишет ЭТОТ скрипт от имени нажавшего «Внести», а не сервис-аккаунт.
 * Жёсткий protected range без всех этих людей в editors сломал бы саму приёмку —
 * то есть защита остановила бы то, что защищает. Диалог «точно правим?» ловит
 * случайное удаление, а намеренное и не должен: у него есть кнопка в меню книги
 * «Склад» («Прибрати історію приймання»), которая безопасна by design.
 */
  ...
  try {
    sheet.protect()
      .setDescription(
        ...
        'переносить залишок. Прибирання — меню «📦 Склад» книги «Склад».')
      .setWarningOnly(true);
  } catch (err) {
    // Защита — удобство, а не условие работы приёмки: падать из-за неё нельзя.
    console.error(err && err.stack ? err.stack : err);
  }
}
