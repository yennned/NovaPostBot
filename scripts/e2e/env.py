...
def load_stand_env(env_file: Path | str | None = None) -> Path:
    ...
    # недоступен, поэтому подменяем его локальным. Холодный кэш справочников НП тут
    # даже полезен: попадание в кэш пропускает резолв ФОП и сделало бы проверку
    # бутафорией (ловушка описана в PROGRESS за 2026-07-15).
    #
    # **Локальный Redis теперь обязателен.** Раньше он обслуживал только кэш НП, и
    # его отсутствие прогон переживал — ошибки кэша глотаются. С переездом FSM в
    # Redis (PR #156) на нём же держится состояние диалога: без него персона не
    # пройдёт дальше первого экрана, требующего ввода.
