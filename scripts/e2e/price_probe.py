...
(`mapping.billable_weight_kg`, коэффициент `объём м³ × 250`). Коэффициент взят из
правил НП и проверен арифметикой на пресетах, но не боевым тарифом. Этот скрипт
закрывает пробел: спрашивает у НП цену за фактический вес и за расчётный объёмный
...
async def main() -> None:
    ...
    try:
        ...
        for preset, dims in SIZE_PRESETS.items():
            ...
            # Перекрёстная проверка коэффициента: спрашиваем НП цену за тот же
            # фактический вес, но с реальными габаритами в OptionsSeat. Если НП
            # вернёт столько же, сколько наш расчёт по объёмной вагі, — значит
            # коэффициент 250 совпадает с её тарификацией, а не угадан.
        ...
        print(
            ...
            "   тими самими габаритами в OptionsSeat, тобто коефіцієнт відтворює її\n"
            "   тарифікацію. ⚠️ означає розбіжність — тоді правиться ця константа\n"
