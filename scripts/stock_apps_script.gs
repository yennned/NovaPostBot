 ...
 *   2. Сохранить. Первая правка «Кількість» попросит авторизацию — разрешить.
 *   3. Проверить: поправить количество → в `_Правки` появилась строка.
 */
...
/**
 * Кто правит. `getActiveUser()` отдаёт адрес не всегда (Google не показывает его
 * при правке из чужого домена), поэтому пусто — штатный исход, а не сбой: строка
 * журнала всё равно ценна временем и величиной правки.
 */
