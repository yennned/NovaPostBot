...
- **Открытые вопросы:** `bootstrap.py:47` повышает до owner **без** проверки статуса
  ...
  до внутренней роли. Черновик `member_of_id` остался в `stash@{0}` как устаревший
  (его вытеснил `76e48af` со схемой `client_accounts`); при попытке вернуть даст
  вторую голову Alembic.
