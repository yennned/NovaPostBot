...
- **Сделано:** приглашение работника по телефону теперь **забирает номер** у
  ...
  аккаунт-пустышку архивирует. Закрыты оба симптома из репорта владельца.
- **Два симптома — одна причина.** (1) «Номер уже пов'язаний з іншим акаунтом» при
  добавлении в команду Вероники. (2) «У работника нет доступа к API, не может
  создавать ТТН от её лица». Второе — следствие первого.
...
- **Отдельного бага «работник без доступа к API» нет — проверено, а не додумано.**
  Штатно приглашённый работник ТТН создаёт: `list_profiles` без `owner_only`, а
  `_resolve_sender` при заданном `account_id` сверяет `profile.account_id`, а не
  `client_id` → работник пользуется ключом НП **владельца аккаунта**. Гейт
  `owner_only` режет только управление ФОП (create/update/set_default) — это
  намеренно (см. запись про `4dcdfc5` ниже) и не трогали. Человек просто сидел в
  собственном пустом аккаунте: ФОП там нет → «ФОП ще не налаштований».
...
- **Дальше:** дождаться мержа → автодеплой прода; владельцу повторить приглашение
  (ручной правки данных не нужно — фикс самолечащий).
