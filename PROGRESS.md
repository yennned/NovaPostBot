...
## 2026-08-01 · fix/alex-cart-edit-replace · «✏️» у кошику правила, а не додавала

### `9dbffd0` — правка позиції замінює кількість
- **Дефект.** `cb_cart_edit` клала поточну кількість позиції в `pending`, а
  `cb_qty_ok` беззастережно сумував `prev + pending["qty"]`. Підтвердження правки
  **без жодної зміни подвоювало позицію** (2 шт → 4), а «−1» від 5 давала 9.
  Режиму правки в стані не існувало — обидва входи вели в один степпер.
- **Чому подорожчав.** Після `abb3e27` роздутий кошик тягне за собою і оголошену
  вартість, і накладений платіж: обидві суми рахуються з `_cart_total(cart)`.
  Клієнт платив страховий збір із подвоєної суми, а з отримувача взяли б удвічі
  більше готівкою.
- **Рішення.** Явний `pending["mode"] == "edit"`, який виставляє тільки
  `cb_cart_edit`; кроки ±/«Макс»/«Ввести число» його не чіпають. Старі `pending`
  з Redis ключа не мають → поведінка «додати», незавершені сесії не ламаються.
- **Стеля степпера** в правці — фактичний залишок: `max(available, entry["qty"])`
  обслуговував адитивну семантику й не давав опустити позицію до реальної
  наявності. Залишок обнулився — алерт і перемальований кошик (як `cb_pick`),
  бо степпер із максимумом 0 непрацездатний.
- **Тести перевірені мутацією:** адитивна логіка валить три з нових, зокрема
  `test_cart_edit_does_not_inflate_insured` — 600 замість 300.
- **Чому дожило до прода.** `cab:ttn:cedit:` не натискав **жоден** пробник:
  єдиний юніт на цьому шляху перевіряв лише `pending["available"]`. `cascade.py`
  тепер заходить у правку й звіряє суму кошика до/після.
- Дальше: `script_stop` в `appleboy/ssh-action` та вирівнювання етапів версій
  (лок-файл, версія Neon, Redis у CI, `release.yml`, SHA-пін appleboy).
