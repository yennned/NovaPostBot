...
## 2026-07-05 · feat/alex-codex-review · авто-ревью PR через OpenAI Codex
- **Зачем:** хотели второй, независимый взгляд на каждый PR помимо CI — Codex
  проходит по диффу и оставляет замечания комментарием.
- **Сделано:** новый воркфлоу `.github/workflows/codex-review.yml`
  (`openai/codex-action@v1`): на `pull_request` (opened/reopened/synchronize)
  прогоняет Codex по диффу PR и постит фидбек комментарием (`actions/github-script`).
  Ключ — в `secrets.OPENAI_API_KEY` (заведён в репозитории, в git не попадает).
  Репозиторий публичный → запуск гейтится `allow-users: yennned,Stepandj`, чтобы
  чужие PR не жгли API-баланс; песочница `permission-profile: ":workspace"`
  (без сети, код PR не выкачивает ключ). `concurrency` отменяет старый прогон
  при частых пушах. Промпт — под контекст проекта, ответ украинский.
- **Дальше:** смёржить в `main` (воркфлоу читается из base-ветки, до мержа на PR
  не срабатывает); при желании убрать `synchronize`, если ревью на каждый пуш — шум.
- **Открытые вопросы:** ключ был прислан в чат открытым текстом — перевыпустить
  в OpenAI-дашборде и обновить секрет.
