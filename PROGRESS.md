...
- **Сделано:**
  ...
  - **C3 (провижн):** `format_view_book` (зовут `attach_view_book`/`provision_client_view_books`):
    `ensure_locale` → `style_stock_worksheet(band_rows=200)` (тёмная шапка, бэндинг на
    щедрый диапазон, CF низкого остатка, автоширина) → `write_available_formula` →
    `build_view_summary`. Новый `build_view_summary` — лист «📊 Зведення»: KPI формулами
    (COUNTA/SUM/SUMPRODUCT по 'Товари') + pivot по категориям (щедрый A1:E1000 ловит
    будущие строки, `filterSpecs NOT_BLANK` прячет пустые). `style_stock_worksheet`
    получил параметр `band_rows` (осн. «Склад» зовёт без него).
  - Тесты: `_view_data_row` (порядок/типы), `_VIEW_HEADERS` порядок. Полный pytest + ruff зелёные.
- **Проверено вживую** на тест-книге: 2 вкладки, заголовки нового порядка, бэндинг 201,
  CF, G-формула считает Доступно, KPI/pivot пересчитываются при записи данных.
- **Известно:** цвет-чипы категорий и точный бэндинг по данным — не при провижне пустой
  книги; бэндинг сделан щедрым (200 строк), низкий остаток подсвечивается всегда.
- **Дальше:** пересобрать тест-бот → E2E: клиент открывает «📦 Товари» → книга оформлена.
