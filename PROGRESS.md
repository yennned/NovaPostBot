...
- **Открытые вопросы:**
  1. **Коэффициент 250 не сверен с боевым тарифом.** Совпадение проверено
     арифметикой на пресетах, а не живым вызовом. Нужен разовый прогон
     `methods.get_price` с `Weight=2` и `Weight=12` на реальном ключе ФОП: цена за
     12 должна совпасть с тем, что теперь показывает карточка для «Великої» + 2 кг.
     У меня нет ключа — прогнать владельцу.
  2. **Предпосылки на сервере для доставки compose** (каталог `./secrets`,
     `APP_IMAGE` в `.env`, отсутствие прод-специфики вне репозиторного файла) —
     сверить перед мержем, SSH у меня нет.
