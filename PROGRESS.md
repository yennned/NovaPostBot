...
- **Сделано:** запрошення працівника по телефону тепер **забирає номер** у
  неактивного користувача — переносить його членство в команду запрошувача, а
  акаунт-лушпиння архівує. Закриті обидва симптоми з репорту власника.
- **Два симптома — одна причина.** (1) «Номер уже пов'язаний з іншим акаунтом» при
  додаванні в команду Вероніки. (2) «У працівника немає доступу до API, не може
  створювати ТТН від її імені». Друге — наслідок першого.
- **Причина.** `UserRepository.create` має `create_account=True` за замовчуванням, і
  `/start`-самореєстрація (`create_pending_client`) цей дефолт не перекриває → **будь-хто,
  хто хоч раз надіслав контакт боту, отримує власний `ClientAccount` + членство
  `account_owner`**. А `client_account_memberships` унікальне по `user_id` → людина
  може бути рівно в одному акаунті. `invite_employee` бачив чуже членство і глухо
  відбивав. «Видалення» клієнта номер не звільняє: це soft-delete
  (`clients._transition` лише гасить статуси, рядок `users` тримає UNIQUE-телефон).
  Тобто незапрошуваним був не тільки «видалений» клієнт, а **будь-який номер,
  що бачив бота** — навіть `pending`, який нічого не робив.
- **Окремого бага «працівник без доступу до API» немає — перевірено, а не додумано.**
  Штатно запрошений працівник ТТН створює: `list_profiles` без `owner_only`, а
  `_resolve_sender` при заданому `account_id` звіряє `profile.account_id`, а не
  `client_id` → працівник користується ключем НП **власника акаунта**. Гейт
  `owner_only` ріже лише керування ФОП (create/update/set_default) — це навмисно
  (див. запис про `4dcdfc5` нижче) і не чіпали. Людина просто сиділа у власному
  порожньому акаунті: ФОП там немає → «ФОП ще не налаштований».
- **Пастка, яку довелося закрити тим же фіксом.** `StartService.register_contact`
  кличе `activate_employee_contact` **тільки** в гілці адопції по телефону
  (`get_by_telegram_id → None`). У самореєстрованого `telegram_id` уже є, тож перша
  гілка повертається раніше — членство `invited` не зняв би вже ніхто, і людина
  застрягла б без акаунта. Тому вступ активуємо на місці: є `telegram_id` → одразу
  `active` (Telegram цей номер уже підтвердив), немає → `invited` як раніше.
- **Захист.** Забрати можна лише неактивного (`user.status is not active`): активний
  клієнт зі своєю справою інакше мовчки став би чужим працівником і зник би зі
  списку «Клієнти» (`list_by_status` тримає тільки `account_owner`). Роль платформи
  перевіряємо ДО членства — у демоутнутого менеджера членство є, і він отримував би
  брехливе «пов'язаний з іншим акаунтом». Акаунт-лушпиння **архівуємо, не видаляємо**:
  `audit_logs.account_id` — FK `ondelete=SET NULL`, видалення стерло б скоуп історії.
- **Как проверено.** 532 теста зелёные (`novapostbot_test`, не dev-база) + 10 в
...
- **Дальше:** дождаться мержа → автодеплой прода; владельцу повторить запрошення
