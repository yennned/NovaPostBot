...
**Фазы 0–6 — в `main`.** Фаза 0 (инфраструктура/каркас), Фаза 1 (данные+RBAC,
...
поддержка по менеджерам). Опц. отложено — PR 9e «Останні отримувачі»; per-manager
аттрибуция ТТН, сводка склада в отчётах, кастомный диапазон/графики. Следующая —
**Фаза 7** (задел CRM/WMS за абстракцией `app/sheets/`), ведёт step.
