...
**Фазы 0–4 — в `main`.** Фаза 0 (инфраструктура/каркас), Фаза 1 (данные+RBAC,
...
`DecryptionError` при ротации `FERNET_KEY`). Опц. отложено — PR 9e «Останні
отримувачі». Следующая — **Фаза 5** (уведомления/трекинг/SLA/возвраты в воркере),
ведёт step.

**Модель работы — sequential-by-phase:** один человек полностью закрывает фазу
(домен + bot/UI), второй ждёт мержа в `main`. Phase 2/4 → alex, Phase 3/5 → step.
