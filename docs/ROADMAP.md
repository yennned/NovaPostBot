...
- **Минимум НП для оголошеної вартості** не проверяется. Сама сумма больше не
  уходит нулём: она выводится из корзины и следует за ней, а при отсутствии цен в
  «Складі» отправка блокируется до явного ввода.
- **Свою сумму накладеного платежу корзина не пересчитывает** — это осознанно
  (её задал клиент), но и переключиться обратно на «суму з кошика» после ручного
  ввода можно только повторным вводом: отдельного экрана выбора у страховой суммы
  нет, в отличие от COD.
